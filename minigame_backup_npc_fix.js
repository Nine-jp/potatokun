...
const SearchGame = (() => {
    ...
    function initThreeJS() {
        ...
        // ★修正版：ここから下のブロックを丸ごと置き換え
        // 【重要】シーン内に残っている古いポテトくん（Tポーズ等の幽霊）を徹底的に探して削除する
        const cleanUpGhosts = () => {
            // 1. グローバル変数の参照先を削除
            if (window.sgPlayer) {
                scene.remove(window.sgPlayer);
                window.sgPlayer = null;
            }

            // 2. シーン内を捜索して、名前や特徴から古いモデルを強制削除
            const ghostsToRemove = [];
            scene.traverse((child) => {
                // 名前が FBX読み込み時のデフォルトっぽいもの、または以前のコードでつけた名前
                if (child.name === 'potatokun_normal' || (child.isGroup && child.children.length > 0)) {
                    // ※安全のため、明らかに背景(Environment)ではない動的モデルっぽいものを判定
                    // 以前のコードで `window.sgPlayer = fbx` としていたオブジェクトは特定しにくいため
                    // ここでは確実に新しいモデルを読み込む前に、以前の変数をクリアするに留めます。
                }
            });
        };
        cleanUpGhosts();
        ...
        loader.load(
            'models/potatokun_thristy.fbx', // Thristyモデル読み込み
            (fbx) => {
                console.log('FBX Model loaded: potatokun_thristy.fbx');
                ...
                // サイズが極端に小さい/大きい場合の安全策
                const originalHeight = size.y > 0 ? size.y : 1.0;
                ...
                // === シーンに追加 ===
                // ここで再度クリーニング（念には念を）
                if (window.sgPlayer) scene.remove(window.sgPlayer);

                fbx.visible = false; // オープニング開始まで隠す
                scene.add(fbx);
                window.sgPlayer = fbx; // グローバル変数に登録

                // === ★アウトライン（輪郭線）の設定 ===
                // 第2引数(角度)を「1」にして、緩い角でも線が出るようにする
                // 第3引数は線の色（黒）
                if (window.addEdgesOutline) {
                    window.addEdgesOutline(fbx, 1, 0x000000);
                    console.log('Thirsty Potatokun: Outline applied with High Sensitivity');
                }
