...
const SearchGame = (() => {
    ...
    function initThreeJS() {
        ...
        */
        ...
        /**
         * 🛠️ PROJECT POTATO: TREE INSTANCING SYSTEM
         * [GOAL] 130本以上の木を1ドローコールに集約し、レンダリング負荷を極限まで削減。
         * [SYSTEM] InstancedMeshManager
         */

        const TREE_PALETTES = {
            spring: [0x90EE90, 0x98FB98, 0xADFF2F],
            summer: [0x228B22, 0x006400, 0x2E8B57],
            autumn: [0xFF8C00, 0xD2691E, 0x8B4513],
            winter: [0xFFFFFF, 0xDDDDDD, 0xCCCCCC]
        };

        const TreeInstanceManager = (() => {
            let instanceMesh = null;
            const treeData = []; // 各インスタンスの初期状態（行列、色）を保持

            /**
             * 1. システムの初期化
             * @param {THREE.Group} masterModel - クローン元となるFBXモデル
             * @param {number} count - 生成する最大本数
             */
            const init = (masterModel, count) => {
                ...
                // モデルからジオメトリとマテリアルを抽出（一番最初のメッシュを使用）
                let targetMesh = null;
                ...
                instanceMesh = new THREE.InstancedMesh(
                    targetMesh.geometry,
                    targetMesh.material.clone(), // 共有マテリアル
                    count
                );
                ...
                instanceMesh.name = "InstancedTreeField";

                scene.add(instanceMesh);
                window.sgTreeInstancedMesh = instanceMesh;
            };

            /**
             * 2. 個別インスタンスのトランスフォーム設定
             * @param {number} index - インスタンス番号
             * @param {THREE.Vector3} pos - 位置
             * @param {THREE.Euler} rot - 回転
             * @param {number} scale - スケール
             */
            const setTransform = (index, pos, rot, scale) => {
        ...
        // グローバルに公開（既存のspawnTrees内などで利用可能にする）
        window.TreeInstanceManager = TreeInstanceManager;

        console.log("🌲 TreeInstanceManager: Ready for mass planting.");
